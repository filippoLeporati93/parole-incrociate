...
async function getStatistics(): Promise<statistics> {
  const statsJson = await AsyncStorage.getItem(STATISTICS_STORAGE_KEY);
  let pref: statistics = {
    games: [],
  };
  if (statsJson != null) {
...
async function addGame(game: game) {
  ...
  await AsyncStorage.setItem(STATISTICS_STORAGE_KEY, JSON.stringify(stats));
}
...
async function calcKpis(timeFilterCode: string, level: number) {
  ...
  if (stats_by_level && stats_by_level.length > 0) {
    ...
    kpis.percGameWon =
      kpis.gamePlayed === 0 ? 0 : kpis.gameWon / kpis.gamePlayed;
    kpis.bestTime = stats_by_level
      .filter(e => e.youWon === true)
      .reduce((a, b) => Math.min(a, b.gameElapsedTime), Infinity);
    kpis.bestTime = kpis.bestTime === Infinity ? 0 : kpis.bestTime;
    kpis.avgTime = Math.round(
      stats_by_level.length > 0
        ? stats_by_level.reduce((a, b) => a + b.gameElapsedTime, 0) /
            stats_by_level.length
        : 0
    );
    kpis.bestScore = stats_by_level.reduce((a, b) => Math.max(a, b.score), 0);
    kpis.avgScore = Math.round(
      stats_by_level.length > 0
        ? stats_by_level.reduce((a, b) => a + b.score, 0) /
            stats_by_level.length
        : 0
    );
    kpis.winningSeries = stats_by_level
      .sort((a, b) => b.gameDatetime.getTime() - a.gameDatetime.getTime())
      .findIndex(e => e.youWon === false);
    kpis.winningSeries =
      kpis.winningSeries === -1 ? kpis.gameWon : kpis.winningSeries;

    let stats_ordered = stats_by_level.sort(
      (a, b) => b.gameDatetime.getTime() - a.gameDatetime.getTime()
    );
    let maxWinningSeries = 0;
    while (stats_ordered.length > 0) {
      let newWinningSeriesIdx = stats_ordered.findIndex(
        e => e.youWon === false
      );
      const series = stats_ordered.splice(
        0,
        newWinningSeriesIdx === -1
          ? stats_ordered.length
          : newWinningSeriesIdx + 1
      );
      maxWinningSeries = Math.max(
        maxWinningSeries,
        newWinningSeriesIdx === -1 ? series.length : series.length - 1
      );
    }
