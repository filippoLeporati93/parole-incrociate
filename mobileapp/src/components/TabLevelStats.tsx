...
const TabLevelStats: React.FC<TabLevelStatsProps> = props => {
  ...
  useEffect(() => {
    StatisticsUtils.calcKpis(props.timeFilterCode, props.gameLevel).then(s => {
      setStats(s);
      setRefreshing(false);
    });
  ...
  return (
    ...
    >
      ...
      />
      <CardStats
        value={(stats.percGameWon * 100).toFixed(0) + '%'}
        icon={'flag-outline'}
      ...
      <CardStats
        value={stats.winningSeries}
        icon={'chevron-right'}
        text={'Serie vincente attuale'}
      />
      <CardStats
        value={stats.bestWinningSeries}
