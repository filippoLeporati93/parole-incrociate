...
const JoinRoomScreeen = ({route, navigation}) => {
  ...
  useEffect(() => {
    SocketService.connect(BASE_WS_URL)
      .then(() => {
    ...
    >
      {noPlayer ? (
        <View style={{flex: 1, justifyContent: 'center'}}>
          ...
          >
            <Text style={styles.panelButtonTitle}>Riprova</Text>
          </Pressable>
        </View>
      ) : (
        <View style={{flex: 1, justifyContent: 'center'}}>
          ...
          <Text
            textType="bold"
            style={{fontSize: 22, textAlign: 'center', marginTop: 20}}
          >
          ...
          <Text style={{fontSize: 40, textAlign: 'center', marginBottom: 1}}>
            {remainingSeconds}
          </Text>
        </View>
      )}
