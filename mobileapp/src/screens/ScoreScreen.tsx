...
const WordLine: React.FC<wordLine> = props => {
  return (
    <View style={{marginHorizontal: 30}}>
      ...
      >
        <View style={{flex: 1}}>
          <Text style={{fontSize: 23}}>{props.wordResults.word}</Text>
        </View>
        <View style={{flex: 1}}>
          <Text textType="light" style={{color: 'gray', textAlign: 'right'}}>
            {props.wordResults.word.length} punti
          </Text>
        </View>
      </Pressable>
      <View
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.pointsContainer}>
        ...
        <Text textType="bold" style={{fontSize: 30, textAlign: 'center'}}>
          {props.result.points}
        </Text>
      </View>
...
const ScoreScreen = ({route}) => {
  const yourResults = route.params.results.filter(
    (e: gameResults) => e.isOpponent === false
  )[0];
  const opponentResults = route.params.results.filter(
    (e: gameResults) => e.isOpponent === true
  )[0];
